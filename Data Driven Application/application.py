...
style.configure('TNotebook.Tab', background="#9fd4a3", width=10)
...
def dictionary():
    ...
    if len(rhymelist) == 0:
        rhymeLabel1["width"] = 100
        rstring1 = "No Input"
    elif len(rhymelist) < 10:
        for x in rhymelist[0:10]:
            rhymeLabel1["width"] = 100
            rstring1 = f"{rstring1} {x}"
            rhymeLabel1.grid(row=1, column=1)
    else:
        for x in rhymelist[0:10]:
            rstring1 = f"{rstring1} {x}"
            rhymeLabel1.grid(row=1, column=1)
        for x in rhymelist[11:20]:
            rstring2 = f"{rstring2} {x}"
...
#HEADER
bg = Frame(root, bg="#9fd4a3")
bg.pack(fill=X)
heading = Frame(bg, bg="#9fd4a3")
heading.pack(side=TOP)
photo = Image.open("logo.png")
img = ImageTk.PhotoImage(photo.resize((60,40)))
logo = Label(heading, image=img, bg="#9fd4a3")
logo.grid(row=1, column=1, pady=10, padx=5)
title = Label(heading, text="TREE OF WORDS", bg="#9fd4a3", font=("PMingLiU-ExtB", 20, "bold"))
...
button = Button(search, width=10, relief=SOLID, bd=1, text="SEARCH", bg="#9fd4a3", fg="white", font=("Times New Roman CE", 10), command=dictionary)
...
line.create_line(0,6,1000,6, fill="#9fd4a3", width=3)
...
antonymWord = Label(antonyms, text="", font=("Times New Roman", 30), width=10, height=1, relief=SOLID, bg="#9fd4a3")
antonymWord.pack(side=TOP)
line2 = Canvas(antonyms, width=960, height=6)
line2.pack(side=TOP, padx=16, pady=(15,10))
line2.create_line(0,6,1000,6, fill="#9fd4a3", width=3)
...
synonymWord = Label(synonyms, text="", font=("Times New Roman", 30), width=10, height=1, relief=SOLID, bg="#9fd4a3")
synonymWord.pack(side=TOP)
line3 = Canvas(synonyms, width=960, height=6)
line3.pack(side=TOP, padx=16, pady=(15,10))
line3.create_line(0,6,1000,6, fill="#9fd4a3", width=3)
...
rhymeWord = Label(rhymes, text="", font=("Times New Roman", 30), width=10, height=1, relief=SOLID, bg="#9fd4a3")
rhymeWord.pack(side=TOP)
line4 = Canvas(rhymes, width=960, height=6)
line4.pack(side=TOP, padx=16, pady=(15,10))
line4.create_line(0,6,1000,6, fill="#9fd4a3", width=3)
rhymeFrame = Frame(rhymes)
rhymeFrame.pack(pady=(10,40))
rhymeLabel1 = Label(rhymeFrame, relief=GROOVE, bd=3, text="", width=50, height=14, font=("Times New Roman", 13), anchor=NW, justify=LEFT)
rhymeLabel2 = Label(rhymeFrame, relief=GROOVE, bd=3, text="", width=50, height=14, font=("Times New Roman", 13), anchor=NW, justify=LEFT)
...
examplesWord = Label(examples, text="", font=("Times New Roman", 35), relief=SOLID, bg="#9fd4a3", width=10)
...
line5.create_line(0,6,1000,6, fill="#9fd4a3", width=3)
